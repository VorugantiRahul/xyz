...
export const PassportCard: React.FC<PassportCardProps> = ({
  ...
  const displayRole = profile?.role || 'Developer';
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="relative rounded-3xl p-[1px] bg-gradient-to-b from-primary/60 via-border to-border/40 shadow-2xl overflow-hidden">
        ...
        <div className="relative bg-surface rounded-3xl p-6 sm:p-10 backdrop-blur-xl space-y-8">
          ...
          {/* Conditional Display: Empty State vs Active Proofs */}
          {skills.length === 0 ? (
            <div className="p-8 sm:p-12 text-center rounded-2xl bg-surface-secondary/50 border border-border space-y-4">
              <div className="w-14 h-14 rounded-2xl bg-surface border border-border flex items-center justify-center text-text-secondary mx-auto shadow-inner-light">
                <Award className="w-7 h-7 text-primary" />
              </div>
              <div className="max-w-md mx-auto space-y-1">
                <h3 className="text-lg font-bold text-text">No Monad Attestations Recorded</h3>
                <p className="text-xs text-text-secondary leading-relaxed">
                  This wallet address has not registered any on-chain skill verifications yet. Complete a practical challenge to mint your first proof.
                </p>
              </div>
              {isOwner && (
                <div className="pt-2">
